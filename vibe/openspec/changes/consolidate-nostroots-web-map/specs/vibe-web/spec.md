...
- **AND** additional experimental experiences (Nostrail, Wikistr, and external
  third-party links such as Treasures and Miti) MUST remain hidden until the
  user enables "Show more experimental apps"
...
- **THEN** the browser-extensions section MUST hide and the download section MAY
  reorder below web experiences (no-op when the download section is already
  hidden in-app).
