...
export function isKnownNip7Method(method: unknown): method is Nip7Method {
  return typeof method === "string" && NIP7_METHODS.includes(method as Nip7Method);
}

export function requestMetadata(rawMessage: string): { id: string; method: string } | null {
  try {
    ...
    switch (request.method) {
      ...
      case "signEvent":
        return success(
          request.id,
          await api.signEvent(
            sanitizeEventTemplate(signEventParamsSchema.parse(request.params)[0]),
          ),
        );
      case "nip44.encrypt": {
