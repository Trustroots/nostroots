...
function createApi(): Nip7BridgeApi {
  return {
    ...
    signEvent: jest.fn(async (eventTemplate: EventTemplate) => ({
      ...eventTemplate,
      id: "2".repeat(64),
      pubkey: validPubkey,
      sig: "3".repeat(128),
...
describe("NIP-07 bridge", () => {
  ...
  it("signs a sanitized event template", async () => {
    const api = createApi();
    const response = await handleNip7BridgeMessage(
      request("signEvent", {
        id: "bad-id",
  ...
  it("routes NIP-44 and NIP-04 calls", async () => {
    ...
    await expect(
      handleNip7BridgeMessage(
        request("nip44.encrypt", [validPubkey, "plain"]),
        api,
      ),
    ).resolves.toMatchObject({ ok: true, result: "nip44-cipher" });
    await expect(
      handleNip7BridgeMessage(
        request("nip44.decrypt", [validPubkey, "cipher"]),
        api,
      ),
    ).resolves.toMatchObject({ ok: true, result: "nip44-plain" });
    await expect(
      handleNip7BridgeMessage(
        request("nip04.encrypt", [validPubkey, "plain"]),
        api,
      ),
    ).resolves.toMatchObject({ ok: true, result: "nip04-cipher" });
    await expect(
      handleNip7BridgeMessage(
        request("nip04.decrypt", [validPubkey, "cipher"]),
        api,
      ),
  ...
  it("returns an error for unknown methods and malformed params", async () => {
    ...
    await expect(
      handleNip7BridgeMessage(request("unknown"), api),
  ...
  it("surfaces invalid peer pubkey errors from crypto helpers", async () => {
    ...
    await expect(
      ...
      ok: false,
      error: "Invalid NIP-44 peer public key.",
    });
  });
});
