...
describe('Browser notifications', () => {
  ...
  describe('Notifications UI', () => {
    ...
    it('has native Nostroots Browser notification bridge support', () => {
      const source = readFileSync(resolve(process.cwd(), 'web/index.js'), 'utf8');

      expect(source).toContain('window.nostrootsBrowser?.notifications');
      expect(source).toContain('subscribePlusCode');
      expect(source).toContain('Native iOS notifications work even when Nostroots Browser is closed.');
    });
  });
});
