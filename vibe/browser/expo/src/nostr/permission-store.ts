...
export async function isRememberedOrigin(origin: string): Promise<boolean> {
  return (await readStringListPreference(SECURE_STORE_NIP7_ALLOWED_ORIGINS_KEY)).includes(
    origin,
  );
}

export async function rememberOrigin(origin: string): Promise<void> {
  const origins = await readStringListPreference(SECURE_STORE_NIP7_ALLOWED_ORIGINS_KEY);
