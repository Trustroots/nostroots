...
function nativeNotificationBridge() {
    return window.nostrootsBrowser?.notifications?.__native
...
async function runNativeNotificationAction(action) {
    const bridge = nativeNotificationBridge();
    if (!bridge) return;
    try {
        nativeNotificationState = await action(bridge);
        notificationTestStatus = null;
    } catch (error) {
        setNotificationTestStatus(error?.message || String(error), 'error');
    }
    renderSettingsNotificationsSection();
    scheduleHeaderKpiRefresh();
}
...
function addNotificationPlusCode(plusCode) {
    ...
    const bridge = nativeNotificationBridge();
    if (bridge) void runNativeNotificationAction((native) => native.subscribePlusCode(plusCode));
    scheduleHeaderKpiRefresh();
    if (document.getElementById('settings-modal')?.classList.contains('active')) {
        renderSettingsNotificationsSection();
    }
...
function removeNotificationPlusCode(plusCode) {
    ...
    const bridge = nativeNotificationBridge();
    if (bridge) void runNativeNotificationAction((native) => native.unsubscribePlusCode(plusCode));
    scheduleHeaderKpiRefresh();
    if (document.getElementById('settings-modal')?.classList.contains('active')) {
        renderSettingsNotificationsSection();
    }
...
function requestNotificationPermission() {
    const bridge = nativeNotificationBridge();
    if (bridge) {
        void runNativeNotificationAction((native) => native.enable());
        return;
    }
...
async function sendTestNotification() {
    const bridge = nativeNotificationBridge();
    if (bridge) {
        await runNativeNotificationAction((native) => native.sendTestNotification());
        setNotificationTestStatus('Test notification sent.', 'info');
        return;
    }
    if (!('Notification' in window)) {
...
function renderSettingsNotificationsSection() {
    ...
    if (nativeBridge) {
        ...
        const codes = state?.subscribedPlusCodes || getNotificationPlusCodes();
        const enabled = !!state?.enabled;
        const tokenStatus = state?.hasToken ? 'APNs token stored' : 'Waiting for APNs token';
        el.innerHTML = `
            <h2>Notifications</h2>
            <p class="notifications-tab-notice" style="color: var(--muted-foreground); font-size: 0.875rem; margin-bottom: 0.75rem;">
                Native iOS notifications work even when Nostroots Browser is closed.
            </p>
            <p style="font-size: 0.875rem; margin-bottom: 0.5rem;">Status: ${enabled ? 'enabled' : 'off'}. ${escapeHtml(tokenStatus)}. Subscribed areas: ${codes.length}.</p>
            ${state?.lastError ? `<p style="font-size: 0.875rem; margin-bottom: 0.5rem; color: var(--destructive);">${escapeHtml(state.lastError)}</p>` : ''}
            <div class="notification-actions">
                ${enabled
                    ? '<button class="btn" onclick="runNativeNotificationAction(native => native.disable())">Disable notifications</button>'
                    : '<button class="btn" onclick="requestNotificationPermission()">Enable notifications</button>'}
                <button class="btn btn-secondary" onclick="sendTestNotification()">Test notification</button>
            </div>
        ...
        `;
        void refreshNativeNotificationState().then((next) => {
            if (next && document.getElementById('settings-notifications-section')) {
                const currentCodes = JSON.stringify(codes);
                const nextCodes = JSON.stringify(next.subscribedPlusCodes || []);
                if (enabled !== !!next.enabled || currentCodes !== nextCodes || tokenStatus !== (next.hasToken ? 'APNs token stored' : 'Waiting for APNs token')) {
                    renderSettingsNotificationsSection();
                }
            }
        });
...
window.runNativeNotificationAction = runNativeNotificationAction;
window.refreshNativeNotificationState = refreshNativeNotificationState;
