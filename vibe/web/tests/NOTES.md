...
## Multi-experience source model

`nr-web` is a static workspace with a root hub and multiple experiences. The current Trustroots Map app is still mostly a two-file bundle:

- `trustroots-map/index.html` — markup, the entire CSS in one `<style>` block, the classic
  helpers (`NrWeb*` globals + `NrWebHashRouter`), and the Keys / Settings
  modals.
- `trustroots-map/index.js` — one ES module containing every Nostroots-authored helper
...
Root `index.js` re-exports the Trustroots Map module for compatibility with unit tests.
...
`trustroots-map/index.js` imports from `https://cdn.jsdelivr.net/npm/...` URLs. jsdom can
...
both the import URL inside `trustroots-map/index.js` and the alias + devDep together.
...
- **Unit tests** import named exports directly from `../../index.js`. They
  ...
  in `tests/unit/`. The Vitest `setup.js` loads `trustroots-map/index.html` into jsdom so
...
- **Tests that need code from `index.html`'s inline `<script>` blocks**
  ...
  Keep these markers in `trustroots-map/index.html` if you reorganise the inlined scripts.
- **Integration tests** drive DOM interactions inside the same jsdom
  fixture.
- **E2E tests** run the real pages in Playwright browsers; Trustroots Map coverage targets `/trustroots-map/`.
