...
DEV_CLIENT_MANIFEST_URL="${DEV_CLIENT_MANIFEST_URL:-http://localhost:8081?disableOnboarding=1}"
...
E2E_PREWARM_ATTEMPTS="${E2E_PREWARM_ATTEMPTS:-2}"
E2E_LAUNCH_SCREEN_ID="${E2E_LAUNCH_SCREEN_ID:-screen-map}"
METRO_STARTED_BY_SCRIPT=0
LOCAL_FLOW_ROOT=""
LOCAL_FLOW_ARGS=""

mkdir -p "${LOG_DIR}" .e2e-screenshots
...
prewarm_app_bundle() {
  ...
  prewarm_flow="${LOCAL_FLOW_ROOT}/prewarm.yaml"
  {
    echo "appId: ${APP_ID}"
    echo "---"
    echo "- openLink: \"${DEV_CLIENT_URL}\""
    emit_open_prompt_tap
    emit_open_prompt_tap
    if [ -n "${E2E_LAUNCH_SCREEN_ID}" ]; then
      echo "- extendedWaitUntil:"
      echo "    visible:"
      echo "      id: \"${E2E_LAUNCH_SCREEN_ID}\""
...
MANIFEST_URL="${MANIFEST_URL:-${DEV_CLIENT_MANIFEST_URL}}"
prewarm_metro_bundle
...
if [ "${E2E_PREWARM_ENABLED}" = "1" ]; then
  echo "Maestro prewarm will launch the development client."
else
...
if [ "${E2E_PREWARM_ENABLED}" = "1" ]; then
  prewarm_attempt=1
  while ! prewarm_app_bundle; do
    capture_hierarchy "${LOG_DIR}/maestro-after-prewarm-failure-${prewarm_attempt}.json"
    if [ "${prewarm_attempt}" -ge "${E2E_PREWARM_ATTEMPTS}" ]; then
    ...
    echo "Retrying app prewarm (${prewarm_attempt}/${E2E_PREWARM_ATTEMPTS})..."
  done
else
  echo "Skipping Maestro prewarm for this run (E2E_PREWARM_ENABLED=${E2E_PREWARM_ENABLED})."
fi
prepare_local_flows "$@"
if [ -n "${MAESTRO_JUNIT_OUTPUT:-}" ]; then
  mkdir -p "$(dirname "${MAESTRO_JUNIT_OUTPUT}")"
fi
if [ -n "${MAESTRO_HOST:-}" ] && [ -n "${MAESTRO_JUNIT_OUTPUT:-}" ]; then
  # LOCAL_FLOW_ARGS is generated from temporary paths without whitespace.
  # shellcheck disable=SC2086
  maestro --host "${MAESTRO_HOST}" test --format junit --output "${MAESTRO_JUNIT_OUTPUT}" ${LOCAL_FLOW_ARGS}
elif [ -n "${MAESTRO_HOST:-}" ]; then
  # LOCAL_FLOW_ARGS is generated from temporary paths without whitespace.
  # shellcheck disable=SC2086
  maestro --host "${MAESTRO_HOST}" test ${LOCAL_FLOW_ARGS}
elif [ -n "${MAESTRO_JUNIT_OUTPUT:-}" ]; then
  # LOCAL_FLOW_ARGS is generated from temporary paths without whitespace.
  # shellcheck disable=SC2086
  maestro test --format junit --output "${MAESTRO_JUNIT_OUTPUT}" ${LOCAL_FLOW_ARGS}
else
  # LOCAL_FLOW_ARGS is generated from temporary paths without whitespace.
  # shellcheck disable=SC2086
